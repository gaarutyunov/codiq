...
**Resolution is bounded by the repository.** The directory an indexing run is pointed at *is* the repository: its manifests are read and no ancestor's. Resolution used to walk upward until something matched, which meant a tree with no manifest of its own inherited whichever one happened to sit above it on that machine — measured, twenty repositories under one home directory taking a single stray `package.json`, rooted at `$HOME`, so that namespaces were rendered relative to home and two unrelated symbols produced byte-identical descriptors. In a database that holds one repository that is a curiosity; in one that holds twenty it materialises `resolves_to` and `calls` edges between unrelated repositories, because §7 joins on the descriptor and nothing else.

**A repository with no manifest is named by its corpus.** Its ecosystems resolve to `<scheme> <manager> <corpus> .` rooted at the repository, rather than failing the run as they used to — which is what makes a Makefile-only C project, an Xcode app, a gemspec-only gem or a `setup.py` tree indexable at all. `.` keeps its meaning for a **version**, which genuinely cannot always be determined, and loses it for a **name**, which now always can. That is what carries corpus isolation into the descriptor rather than leaving it at the `file` row: `file.corpus` alone would keep the rows apart and still let the link pass join them.

The consequence, stated because it is the one user-visible behaviour change: `codiq ./subdir` inside a module no longer inherits that module's coordinate. That usage never produced a coherent graph — `file.path` was relative to the subdirectory while the coordinate's root was the module, so paths and namespaces were measured from different origins — so it is a narrowing of something already broken rather than the loss of something that worked.
...
|------------|------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------|
|`file`      |`file_id` PK; `corpus`, `path`, `lang`; `pkg_scheme`, `pkg_manager`, `pkg_name`, `pkg_version`                                      |file identity is `(corpus, path)`; package coordinate = descriptor prefix source|
...
**File identity is `(corpus, path)`, not `path`.** One database holds many repositories, and `src/main.go` is a path most of them have — so the loader resolves a file row on the pair, and two repositories' same-named files are two rows with two ids. `corpus` is named by the indexing run (`codiq -corpus <name>`, defaulting to the repository directory's name) and is a property of the run rather than of the file's bytes, so no extractor sees it.

Neither column carries a unique constraint, only a btree each. `@key(fields: ["corpus", "path"])` is expressible now that a corpus exists, and is deliberately not taken; see §16.
...
- **A `(corpus, path)` unique key on `file`.** Not an oversight, and worth recording because it stopped being impossible. The SDL declared only a btree on `path` and said so on the grounds that "uniqueness of a path is a property of a *corpus*, and the corpus boundary is not modelled" — that reason expired when `corpus` landed (§4.4), and `@key(fields: ["corpus", "path"])` is expressible today. It is still not taken. What it would buy is one thing: the loader's file resolve could become an `ON CONFLICT` upsert and drop its per-file advisory lock. That is a load-path optimisation with no correctness argument behind it — the lock already makes the select-then-insert race-free, and it is now keyed on the pair so two corpora's same-named files do not serialise on each other. Taking the key would also make a duplicate a *run failure* rather than a row the next re-index replaces, which is a stricter contract than anything currently needs. The measurement that would justify it is the lock showing up in a profile of a many-repository run.
- **Corpus-scoped link rebuilds.** `link.RebuildAll` recomputes the derived edges over the whole database, so two indexers over *different* repositories still race (the foreign-key violation `cmd/codiq`'s `start` records), and one repository's re-index pays for every other's edges. Once resolution is bounded by the repository (§4.3) a whole-graph rebuild is *correct* — a descriptor cannot span two corpora — merely slower than it needs to be. Correctness first; scoping it belongs with the incremental link (§14 M8) and needs a measurement rather than an assumption.